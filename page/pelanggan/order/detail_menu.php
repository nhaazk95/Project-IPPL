<?php
    ...
    if (isset($_POST['btnTambah'])) {
        ...
        $keterangan    = trim($_POST['keterangan'] ?? ''); // FIX: ambil catatan dari form
        ...
        if ($total <= 0 || $sub_total <= 0) {
            ...
            if ($num > 0) {
                ...
                $dm->update("tb_order", "status_order='belum_bayar'", "kd_order", $data_kd, "?page=detail_menu&kategori=$kate&kd=$kd");
            }

            // FIX: Simpan catatan jika diisi
            if (!empty($keterangan)) {
                $keterangan = mysqli_real_escape_string($con, $keterangan);
                $valCat = "keterangan='$keterangan', status_keterangan='N'";
                $dm->update("tb_detail_order_temporary", $valCat, "order_kd", $data_kd, "?page=detail_menu&kategori=$kate&kd=$kd");
                $dm->update("tb_detail_order", $valCat, "order_kd", $data_kd, "?page=detail_menu&kategori=$kate&kd=$kd");
            }
        }
    }
...
.dm-header {
    ...
    box-shadow:0 3px 16px rgba(62,31,0,0.3);
...
.dm-header .cart-btn {
    ...
    box-shadow:0 3px 12px rgba(200,151,58,0.4);
}
...
.btn-tambah-keranjang {
    display:block; width:calc(100% - 28px); margin:14px 14px 0;
...
.keranjang-modal-overlay {
    ...
    background:rgba(62,31,0,0.55); align-items:flex-end;
}
...
<!-- Keranjang Modal (read-only, hanya lihat isi keranjang) -->
    ...
    <form method="post">
        <div class="dm-form-card">
            ...
            <div class="catatan-field">
                ...
                <div class="catatan-hint">Catatan akan langsung tersimpan saat tambah ke keranjang</div>
            </div>
        </div>
