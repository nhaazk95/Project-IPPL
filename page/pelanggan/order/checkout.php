<?php
    ...
    $pesanan = [];
    if ($data_kd) {
        $sqlP = "SELECT t.*, m.name_menu FROM tb_detail_order_temporary t
    ...
    // Jika $data_kd masih null, coba ambil dari session
    if (!$data_kd) {
        ...
        // Hitung ulang total jika data_kd baru didapat
        if ($data_kd && $total_bayar == 0) {
            ...
            // Ambil pesanan juga
            $sqlP2 = "SELECT t.*, m.name_menu FROM tb_detail_order_temporary t
                      LEFT JOIN tb_menu m ON t.menu_kd = m.kd_menu
                      WHERE t.order_kd = '$data_kd'";
...
.co-ring-item {
    ...
    font-size:13px; padding:6px 0; border-bottom:1px dashed #f0f0f0;
}
...
.co-ring-total {
    ...
    margin-top:12px; padding-top:12px; border-top:2px solid #f0f0f0;
}
...
.qr-steps  { text-align:left; background:var(--bg); border-radius:12px; padding:14px 16px; font-size:13px; }
...
.kasir-msg {
    ...
    font-size:15px; font-weight:700; margin-bottom:16px; line-height:1.6;
}
.kasir-total { font-size:28px; font-weight:900; color:var(--primary); margin-bottom:6px; }
.kasir-sub   { font-size:13px; color:#888; margin-bottom:20px; }
.kasir-info  {
    background:#fff8e6; border-radius:12px; padding:14px 16px;
    text-align:left; font-size:13px; color:#856404; font-weight:600;
}

.btn-back {
    display:block; margin:16px; padding:14px;
...
/* Error state */
.co-error {
    margin:40px 16px; text-align:center; padding:40px 20px;
    background:var(--card-bg); border-radius:var(--radius); box-shadow:var(--shadow);
    color:#888;
}
...
<div class="co-wrap">
    ...
    <div class="co-metode-list">
        ...
        <a href="?page=checkout&kd=<?= urlencode($data_kd) ?>&total=<?= $total_bayar ?>&metode=qris"
           class="metode-card">
            <div class="metode-icon qr">📱</div>
            <div class="metode-info">
                <h5>QRIS</h5>
                <p>Bayar via QR Code (GoPay, OVO, Dana, dll)</p>
            </div>
            <i class="fa fa-chevron-right metode-arrow"></i>
        </a>

        <a href="?page=checkout&kd=<?= urlencode($data_kd) ?>&total=<?= $total_bayar ?>&metode=kasir"
           class="metode-card">
            <div class="metode-icon kasir">🏪</div>
            <div class="metode-info">
                <h5>Kasir</h5>
                <p>Bayar langsung ke kasir (tunai/debit)</p>
            </div>
            <i class="fa fa-chevron-right metode-arrow"></i>
        </a>
    ...
    <div class="qr-panel">
        ...
        <div class="qr-img">
            <!-- Ganti dengan gambar QRIS kamu: <img src="images/qris.png" alt="QRIS"> -->
    ...
    <div class="kasir-panel">
        ...
        <div class="kasir-msg">
            Silahkan melakukan pembayaran di kasir.<br>
            Tunjukkan halaman ini kepada petugas kasir.
        </div>
        ...
        <div class="kasir-info">
            ...
            Harap siapkan uang pas untuk mempercepat proses.
        </div>
    </div>
    <a href="?page=transaksi" class="btn-back">
        <i class="fa fa-arrow-left"></i> Kembali ke Keranjang
